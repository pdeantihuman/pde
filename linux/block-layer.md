...
在每个给定的块 I/O 操作中，在 `bio_vec` 阵列中有从 `bio_vec` 开始的双矢量。当执行块 I/O 操作时，bi\_idx字段用于指向数组中的当前索引。
...
deadline I/O 调度程序试图防止 Linus 电梯引起的饥饿。为了最大限度地减少寻道，对磁盘一个区域的繁重磁盘 I/O 操作会无限期地使对磁盘另一部分的请求操作挨饿。事实上，对磁盘同一区域的请求流可能导致其他远程请求永远不会被服务器控制。这种饥饿是不公平的。

更糟糕的是，请求饥饿的一般问题引入了一个称为写饥饿读取的问题的具体实例。每当内核绕过写操作时，写操作通常都可以提交到磁盘，与子提交应用程序完全异步。
读取操作完全不同。通常，当应用程序提交读请求时，应用程序会阻塞，直到请求得到满足。也就是说，读取请求与提交的应用程序同步发生。虽然系统响应很大程度上不受写延迟(提交写请求所需的时间)的影响，但读延迟(提交读请求所需的时间)很重要。写延迟对应用程序性能几乎没有影响，但是应用程序必须等待每个读请求的完成。因此，读取延迟对系统性能很重要。

使问题复杂化的是，读请求往往相互依赖。例如，考虑读取大量文件。每次读取都以小缓冲块的形式进行。
应用程序不会开始读取下一个块(或下一个文件)，直到从磁盘读取前一个块并将其返回给应用程序。
更糟糕的是，读取和写入操作都需要读取各种元数据，例如索引节点。
从磁盘上读取这些块会进一步序列化 I/O。
因此，如果每个读取请求都被单独饿死，那么对这些应用程序的总延迟就会增加，并且会变得非常大。
认识到读请求的异步性和相互依赖性导致读延迟对系统性能的影响更大， deadline I/O 调度器实现了几个特性，以确保一般的请求匮乏和具体的读匮乏被最小化。

请注意，减少请求不足是以牺牲全局吞吐量为代价的。即使 Linus 电梯也做出了这种妥协，尽管方式要温和得多。如果Linus 电梯总是按扇区将请求插入队列，并且从不检查旧的请求，而是恢复到队列末尾的插入，那么它可以提供更好的整体吞吐量(通过更大程度地最小化寻道)。尽管最小化寻找很重要，但无限期饥饿也不好。因此， deadline  I/O 调度器在提供良好全局吞吐量的同时，更加努力地限制饥饿。毫无疑问:提供请求公平，同时最大化全局吞吐量是一项艰难的工作。

在deadline I/O 调度程序中，每个请求都与一个到期时间相关联。默认情况下，读取请求的过期时间为500毫秒，写入请求的过期时间为5秒。deadline I/O 调度程序的操作类似于 Linus 电梯，因为它维护一个按磁盘上的物理位置排序的请求队列。它将这个队列称为排序队列。当一个新的请求被提交到排序后的队列时，deadline I/O 调度器像 Linus 电梯一样执行合并和插入。然而，deadline I/O 调度器也根据请求的类型将请求插入第二个队列。读请求被分类到一个特殊的读FIFO队列中，写请求被插入到一个特殊的写FIFO队列中。虽然正常队列是按磁盘扇区排序的，但这些队列是FIFO的。(实际上，它们是按时间排序的。)因此，新的请求总是被添加到队列的尾部。在正常操作下，deadline I/O 调度程序将请求从排序队列的头部拉入调度队列。调度队列然后被馈送到磁盘驱动器。这导致最小的寻道。

如果写FIFO队列或读FIFO队列头部的请求到期(即，如果当前时间变得大于与该请求相关联的到期时间)，则deadline I/O 调度器开始服务来自FIFO队列的请求。以这种方式， deadline  I/O 调度器试图确保没有请求的未完成时间超过其到期时间。

请注意，deadline I/O 调度程序对请求延迟没有任何严格的保证。然而，它通常能够在请求到期时或到期前提交请求。这防止了请求饥饿。因为读请求的到期值比写请求的到期值小得多，所以deadline I/O 调度器还可以确保写请求不会使读请求挨饿。这种对读取请求的偏好提供了最小的读取延迟。